...
<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js"></script>
  ...
  <script>
			...
			$dateQuery=mysql_query("SELECT ue.*,e.* FROM `useequipment` ue,`equipment` e where ue.equipment_ID=e.equipmentid and ue.start_date>now() and ue.last_date>now() and ue.status='1' and e.equipmentid='$id'");
	...
	$descQuery=mysql_query("SELECT ue . * , e . * , u . *,c.first_name,c.last_name FROM `useequipment` ue, `equipment` e, `usertable` u,customer c WHERE ue.`equipment_ID` = e.`equipmentid` AND ue.customer_ID=c.customerId AND ue.`user_ID` = u.`userID` AND ue.`start_date`='$lastdate' and e.`equipmentid`='$id'");
