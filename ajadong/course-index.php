...
<html lang="en">
  ...
  <body id="course-index">
    ...
    <section class="course-index padding-default full-width container">
	  ...
	  <?php
	  $numquery=mysql_query("SELECT s.*,u.universityName FROM subjecthighschool s,university u where s.universityID=u.universityId");
	  $num=mysql_num_rows($numquery);
		$limit=12; // limit of contant per page
		$start=0;
		// this is for total contant in a page
		$total=intval($num/$limit);
		if(($num%$limit)>0){
		$total=$total+1;
		}
		$page   = (isset($_GET['page']))? intval($_GET['page']): 1;
		$tpages = (isset($_GET['tpages'])) ? intval($_GET['tpages']) : $total; // total number of pages
		$adjacents  = (isset($_GET['adjacents'])) ? intval($_GET['adjacents']) : 4;
		if($page<=0)  $page  = 1;
		...
		$appQuery=mysql_query("SELECT s.*,u.universityName FROM subjecthighschool s,university u where s.universityID=u.universityId order by videoCount DESC LIMIT $start , $limit");
