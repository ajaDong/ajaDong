...
//-----------------------------------------------------------------------------------------
			...
			$querypay=mysql_query("SELECT cv.*,c.chapterName,s.* FROM courseVideo cv, chapterSubject c,subjecthighschool s WHERE cv.chapterID=c.chapterId and cv.courseID=s.subjectId and cv.videoId='$id' and s.subjectId='$sbid'");
			$countpay=mysql_num_rows($querypay);
			$respay=mysql_fetch_object($querypay);
			$_SESSION['totalprice']=$respay->coursePrice;
			$_SESSION['videoId']=$respay->videoId;
			$_SESSION['subjectID']=$sbid;
			$realPrice=$respay->coursePrice;
			
//--------------------------------------------------------------------------------------------

if(isset($_POST['cal']))
	{
if($_POST['couponnumber']<>'')
	{
	$varifyQuery=mysql_query("SELECT * FROM courseCoupon where CouponsNumber='".addslashes($_POST['couponnumber'])."' and courseId='$sbid'");
	$result=mysql_fetch_object($varifyQuery);
	$DiscountPrice=$result->percentAmount;
	$FinalPrice1 =($DiscountPrice / 100)*$realPrice;
	$FinalPrice=$realPrice - $FinalPrice1;
    $_SESSION['totalprice']=$FinalPrice;
	header('Location:payvideo.php');
}
else
{
   header('Location:payvideo.php');
...
<section class="my-profile-content no-padding container" id="my-profile">
  ...
  <tr>
    <td colspan="4" align="right">
		...
		<div id="couponnumber" style="display: none">
		<input type="text" name="couponnumber"  placeholder="Enter your coupon number..">
        <input type="submit" class="go-red-button"    value="GO" name="cal">
        </div>
        
	Have any coupon ? Check Here
	...
	<input type="hidden" value="<?php echo $_SESSION['userid'];?>" name="id">
			...
			<input type="submit" class="go-red-button"  id="btnSubmit" value="PAY" name="pay6">
