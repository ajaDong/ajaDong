...
<html lang="en">
  ...
  <head>
    ...
    <script>
function showSeller(val)
{
//alert(val);
$("#display").html();
if(val!='' && val.length>0)
{
$.ajax({type: "POST",url: "universitysuggestion.php",data:{val:val},cache: false,success: function(html)
{
//alert(html);
...
<form action="search.php" method="post" enctype="multipart/form-data">
	...
	<div class="col-lg-11"><input type="text" class="sandeep" name="uni" id="sub" onKeyUp="showSeller(this.value)" placeholder="Search University..." required>
    <table class="table table-hover table-striped" id="display">

</table> 
    </div>
    <div class="col-lg-1 margin-left"><span class="input-group-btn">
    ...
    <section class="content-top-course container">
      ...
      <div class="content-inside row">
		...
		<?php
		//$appQuery=mysql_query("SELECT cv.*,u.universityName,s.*,t.firstnameTutor FROM courseVideo cv,university u,subjecthighschool s,tutorhighschool t where cv.uiversityID=u.universityId  and cv.courseID=s.subjectId and cv.userID=t.tutorId order by videoCount DESC LIMIT 6");
		
		$appQuery=mysql_query("SELECT s.*,u.universityName FROM subjecthighschool s,university u where s.universityID=u.universityId order by videoCount DESC LIMIT 6");
...
.table{ text-align:left; text-shadow:none; background:#fff; color:#000; cursor:pointer;} .margin-left{margin-left:-29px;}
</style>
  </body>
