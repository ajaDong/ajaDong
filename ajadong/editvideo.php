...
$query=mysql_query("select * from tutorhighschool where tutorId='".$_SESSION['userid']."'");
		  ...
		  $query=mysql_query("SELECT cv.*,s.*,c.* FROM courseVideo cv,subjecthighschool s,chapterSubject c WHERE cv.userID='".$_SESSION['userid']."' and cv.chapterID=c.chapterId and cv.courseID=s.subjectId and cv.videoId='$videoid'");
		  $res=mysql_fetch_object($query);
		  $capt=$res->chapterID;
		  $sub=$res->subjectID;
...
if($_POST['courseID']<>'' && $_POST['chapterID']<>''  && $_FILES['system']['name']<>'' && $_FILES['system']['size']>0)

		{
			...
			$res=mysql_query("UPDATE `courseVideo` SET `uiversityID`='".$ui."',`courseID`='".addslashes($_POST['courseID'])."',`userID`='".$_SESSION['userid']."',`priceStatus`='".addslashes($_POST['priceStatus'])."',`video`='".$v_video."',`videoType`='".$vfileext1."',`pdfupload`='".$image_file2."' WHERE videoId='$videoid'");
			mysql_query("UPDATE `chapterSubject` SET `chapterName`='".addslashes($_POST['chapterID'])."' WHERE `chapterId`='$capt'");
}else {
        ...
        <section class="content">
          ...
          <div class="box">
			...
			<select name="courseID"  class="form-control" onChange="getsubCat(this.value)" style="width:50%;" required>
					  ...
					  ?>
					  	<option <?php if($mediaRes1->subjectId==$sub) { echo 'selected="selected"';}?> value="<?php echo $mediaRes1->subjectId;?>"><?php echo $mediaRes1->subjectName;?></option>
						<?php 
					  }
					  }
					  ?>
			 		 ...
			 		 <div class="form-group">
                     <?php
					 $userQuery=mysql_query("select * from chapterSubject where chapterID='$capt'");
					 $userRes=mysql_fetch_object($userQuery);
					  ?>

                      <label for="exampleInputPassword1">Chapter Name<span>*</span></label><br/>
 					
                    <input type="text" class="form-control" id="chapterID" name="chapterID" style="width:50%;"  value="<?php echo $userRes->chapterName;?>" required>
					   ...
					   <!--<select name="chapterID" id="subcategory" class="form-control" style="width:50%;" required>
							...
							while($userRes=mysql_fetch_object($userQuery))
							 ...
							 ?>
							 <option  <?php if($userRes->chapterId==$res->chapterID) { echo 'selected="selected"';}?> value="<?php echo $userRes->chapterId;?>"><?php echo $userRes->chapterName;?></option>
							 <?php }}?>
                        </select>-->
                      ...
                      <label for="exampleInputPassword1">Topic Name<span>*</span></label><br/>
					   ...
					   <input type="text" class="form-control" id="videoName" name="videoName" style="width:50%;"   placeholder=" Enter Video  Name ." value="<?php echo $res->videoName;?>" required><br/>
					  </div>-->
			...
			<label for="exampleInputPassword1">video Free<span>*</span></label>
			   ...
			   <input id="check88" type="checkbox" name="priceStatus" value="1" >
                    ...
                    <div class="col-lg-12"><input type="submit" class="go-red-button"  id="btnSubmit" value="Submit" name="addV">&nbsp;&nbsp;<a href="allchaptershow.php"><input type="button" class="go-red-button"  id="btnSubmit" value="Cancel"></a></div>
