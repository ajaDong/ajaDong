...
{
mysql_query("UPDATE courseVideo SET videoCount=videoCount+1 WHERE videoId='$id'");
...
<html lang="en">
  ...
  <body id="subject">
    ...
    <?php include_once("includes/header.php"); ?>
	 <?php
		$appQuery=mysql_query("SELECT cv.*,u.universityName,s.*,t.* FROM courseVideo cv,university u,subjecthighschool s,tutorhighschool t where cv.uiversityID=u.universityId and cv.courseID=s.subjectId and cv.userID=t.tutorId and cv.courseID='$sbid' and cv.videoId='$id'");
    ...
    <section class="content-subject container">
      <div class="content-inside col-md-12">
        ...
        <div class="title-header header-padding grey-bg-wrapper col-md-12 container">
          ...
          <div class="subject-buttons col-xs-12">
			...
			?>
			<?php if($_SESSION['userid']<>'' && $subjectid==$sbid){  ?>
			 <div class="btn-custom full-width succeed">
                  <i class="fa fa-check"></i><p>Already Enrolled</p>
                </div>
			
			<?php }else if($_SESSION['userid']<>''){ ?>
              <a href="payvideo.php?videoid=<?php echo base64_encode($resvideo->videoId);?>&sbid=<?php echo base64_encode($sbid);?>">
              ...
              <div class="panel panel-default">
                <div class="panel-heading">
                  <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#materi-1">
                    <h4>
                      Week 1. <?php echo $reschapter->chapterName;?>
      ...
      data-setup="{}">
     	...
     	<source style="margin-top:20px; width:100%; height:365px;" src="videos/<?php echo $v_video;?>" type='video/<?php echo $resvideo->videoType;?>' />
				...
				if($_SESSION['userid']<>''){
				?>
				<a href="payvideo.php?videoid=<?php echo base64_encode($resvideo->videoId);?>&sbid=<?php echo base64_encode($sbid);?>"  class="go-red-button">Paid Video</a>
				
				
				<?php }else{?>
				<a href="login.php?videoid=<?php echo base64_encode($resvideo->videoId);?>" class="go-red-button">Paid Video</a>
				<?php } }?>
              </div>
			  <?php }}?>
